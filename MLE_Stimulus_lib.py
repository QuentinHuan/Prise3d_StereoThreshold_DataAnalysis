...
def newObservation(spp, k, x0):
    p_error = 0.0  # bit switching error probability
...
def MLE_simulation(x0_ref, k_ref, N):
    ...
    for i in range(N):
        ...
        obs = newObservation(X[-1], k_ref, x0_ref)
        Y.append(obs)
...
def test_MLE_procedure(B, N, bShowPlots):
    ...
    if bShowPlots == True:
        ...
        plt.show()
    return np.mean(abs(np.asarray(ERROR_X0_MLE)))


print("begin performance Analysis")
# result test_MLE_procedure(1,15,True)

B = 100
L = [x for x in range(10, 30)]
result = []
for N in L:
